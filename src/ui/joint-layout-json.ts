...
export default class GraphLayout {
  ...
  // index by ID
  cells: Map<string, Machine | Link | State>;
  data: INetworkJson;
  ...
  constructor(source_graph: joint.dia.Graph, options = {setLinkVertices: false}) {
    ...
    this.cells = new Map<string, Machine | Link | State>()
  }

  fromJson(data: INetworkJson, changed_cells: (Link | State | Machine)[] = []) {
    let start = Date.now()
  ...
  layout() {
    ...
    for (let [id, graph] of this.subgraphs.entries()) {
      if (!graph.graph().is_dirty)
        continue
      // try to reuse the layout from identical graphs
      // TODO optimise, index by hash
      for (let graph_inner of this.subgraphs.values()) {
        if (graph_inner == graph || graph_inner.graph().is_dirty)
          continue
  ...
  // TODO split!
  syncData(data: INetworkJson, changed_cells: (Link | State | Machine)[] = []) {
    ...
    for (let cell of data.cells) {
      ...
      // new cluster
      if ((cell as Machine).embeds && !subgraphs.get(cell.id)) {
        cell = cell as Machine
      ...
      } else if ((cell as State).type == "fsa.State") {
        cell = cell as State
        cells.set
        let [parent_id, id] = cell.id.split(':')
        parent_id = this.normalizeId(parent_id)
        if (!subgraphs.get(parent_id).node(id) || changed_cells.includes(cell.id)) {
      ...
      } else if ((cell as Link).type == "fsa.Arrow") {
        cell = cell as Link
  ...
  syncSourceGraph(data: INetworkJson, changed_cells: (Link | State | Machine)[] = []) {
