...
import * as assert from 'cola-network'
import {
...
export function objectHash(obj: State | Link | Machine) {
  let key
  if (isTypeMachine(obj)) return obj.id
  else if (isTypeState(obj)) return `${obj.machine_id}:${obj.name}`
  else if (isTypeLink(obj)) return `${obj.source_name}:${obj.target_name}`
